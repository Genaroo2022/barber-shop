...
@Component
public class HaircutAiRateLimiter {

    private final int maxPerMinute;
    private final int maxPerHour;
    private final Map<String, Deque<Long>> attemptsByIp = new ConcurrentHashMap<>();

    public HaircutAiRateLimiter(
            @Value("${app.security.ai.max-requests-per-minute:4}") int maxPerMinute,
            @Value("${app.security.ai.max-requests-per-hour:24}") int maxPerHour
    ) {
    ...
    public void checkAllowed(String clientIp) {
        ...
        if (attemptsInLastMinute >= maxPerMinute) {
            throw new TooManyRequestsException("Demasiadas pruebas de IA en poco tiempo. Intenta nuevamente en 1 minuto.");
        }

        if (attempts.size() >= maxPerHour) {
            throw new TooManyRequestsException("Demasiadas pruebas de IA desde tu IP. Intenta nuevamente mas tarde.");
        }
    }
